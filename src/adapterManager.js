...
const deepCloneFilter = (obj, result, key, clone) => {
...
function getAdUnitCopyForPrebidServer(adUnits, s2sConfig) {
  let adUnitsCopy = deepClone(adUnits, deepCloneFilter);
...
function getAdUnitCopyForClientAdapters(adUnits) {
  let adUnitsClientCopy = deepClone(adUnits, deepCloneFilter);
...
adapterManager.makeBidRequests = hook('sync', function (adUnits, auctionStart, auctionId, cbTimeout, labels, ortb2Fragments = {}, auctionMetrics) {
    ...
    if (s2sConfig && s2sConfig.enabled) {
      ...
      serverBidders.forEach(bidderCode => {
        ...
        const bidderRequest = addOrtb2({
          ...
          bidderRequestId,
          uniquePbsTid,
          bids: hookedGetBids({bidderCode, auctionId, bidderRequestId, 'adUnits': deepClone(adUnitsS2SCopy, deepCloneFilter), src: CONSTANTS.S2S.SRC, metrics}),
  ...
  clientBidders.forEach(bidderCode => {
    ...
    const bidderRequest = addOrtb2({
      ...
      auctionId,
      bidderRequestId,
      bids: hookedGetBids({bidderCode, auctionId, bidderRequestId, 'adUnits': deepClone(adUnitsClientCopy, deepCloneFilter), labels, src: 'client', metrics}),
      auctionStart: auctionStart,
      timeout: cbTimeout,
      refererInfo,
      metrics,
