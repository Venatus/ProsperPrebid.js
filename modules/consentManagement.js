  ...
  function handleV1CmpResponseCallbacks() {
    ...
    return {
      consentDataCallback: function (consentResponse) {
        logInfo(`CMP framework received consentCallback: `, consentResponse);
        cmpResponse.getConsentData = consentResponse;
        afterEach();
      },
      vendorConsentsCallback: function (consentResponse) {
        logInfo(`CMP framework received vendorCallback: `, consentResponse);
...
export function setConsentConfig(config) {
  ...
  if (isBoolean(config.enablePolling)) {
