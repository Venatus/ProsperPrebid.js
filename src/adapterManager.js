...
adapterManager.makeBidRequests = hook('sync', function (adUnits, auctionStart, auctionId, cbTimeout, labels) {
  ...
  clientBidderCodes.forEach(bidderCode => {
    ...
    const bidderRequest = {
      ...
      auctionId,
      bidderRequestId,
      bids: hookedGetBids({bidderCode, auctionId, bidderRequestId, 'adUnits': utils.deepClone(adUnitsClientCopy, (obj, result, key, clone) => {
          if (key[0] == '_') {//props prefixed an underscore, will not be cloned!
            result[key] = obj[key];
            return true;
          }
          return false;//normal clone
        }), labels, src: 'client'}),      auctionStart: auctionStart,
      timeout: cbTimeout,
      refererInfo
    };
