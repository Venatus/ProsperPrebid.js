...
adapterManager.makeBidRequests = hook('sync', function (adUnits, auctionStart, auctionId, cbTimeout, labels) {
    ...
    if (s2sConfig && s2sConfig.enabled) {
      ...
      serverBidders.forEach(bidderCode => {
        ...
        const bidderRequest = {
          ...
          bidderRequestId,
          uniquePbsTid,
          bids: hookedGetBids({bidderCode, auctionId, bidderRequestId, 'adUnits': deepClone(adUnitsS2SCopy, deepCloneFilter), src: CONSTANTS.S2S.SRC}),
  ...
  clientBidders.forEach(bidderCode => {
    ...
    const bidderRequest = {
      ...
      auctionId,
      bidderRequestId,
      bids: hookedGetBids({bidderCode, auctionId, bidderRequestId, 'adUnits': deepClone(adUnitsClientCopy, deepCloneFilter), labels, src: 'client'}),
      auctionStart: auctionStart,
      timeout: cbTimeout,
      refererInfo
    };
