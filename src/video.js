...
export const checkVideoBidSetup = hook('sync', function(bid, adUnit, videoMediaType, context, useCacheKey) {
  ...
  if (context === OUTSTREAM && !useCacheKey) {
    return videoMediaType.requireRenderer === false || !!(bid.renderer || (adUnit && adUnit.renderer) || videoMediaType.renderer);//allow to by-pass renderer requirement as ad-manager can handle this internally
  }
