...
const ID_REQUEST = {
  buildServerRequests(basicRequest, bidRequests, bidderRequest) {
    ...
    const requests = [];
    const singleRequestMode = DEFAULT_SINGLE_REQUEST || config.getConfig('improvedigital.singleRequest') === true;
  ...
  buildBannerRequest(bidRequest) {
    ...
    if (config.getConfig('improvedigital.usePrebidSizes') === true && bidRequest.sizes) {
      if (bidRequest.params.overwriteSizes && isArray(bidRequest.params.overwriteSizes)) {
        banner.format = bidRequest.params.overwriteSizes.map(sizePair => ({w: sizePair[0], h: sizePair[1]})); ;
      } else {
        // Convert sizes from [x, y] to { w: x, h: y}
        banner.format = bidRequest.sizes.map(sizePair => ({w: sizePair[0], h: sizePair[1]}));
      }
    } else if (bidRequest.params && bidRequest.params.mergeSizes && bidRequest.params.mergeSizes.length > 0) {
      banner.format = bidRequest.params.mergeSizes;
      if (bidRequest.sizes && bidRequest.sizes.length > 0) {
        banner.format = banner.format.concat(bidRequest.sizes);
      }
      banner.format = banner.format.map(sizePair => ({w: sizePair[0], h: sizePair[1]}));
    }
    return banner;
  },
